...
function readConfig(app: any): { bundled?: boolean; cdn?: string } {
  return (ChartJsPlugin as any).config ?? app.config?.plugins?.['@ux3/ux-chart-js'] ?? {};
}
  ...
  install(app) {
    ...
    app.registerService?.('chart', () => ({
      async create(el: HTMLElement | string, config: Record<string, unknown>) {
        ...
        if (!canvas) throw new Error('@ux3/ux-chart-js: target element not found');
