...
export class UxFilterBuilder extends UxBase {
  ...
  private add(): void {
    ...
    this.render();
    this.dispatchEvent(new CustomEvent('ux:filter.action', { bubbles: true, composed: true, detail: { action: 'ADD', id } }));
  }

  private removeFilter(id: string): void {
    this.filters = this.filters.filter(f => f.id !== id);
    this.render();
    this.dispatchEvent(new CustomEvent('ux:filter.action', { bubbles: true, composed: true, detail: { action: 'REMOVE', id } }));
  }

  private update(id: string, key: keyof Filter, value: string): void {
    const f = this.filters.find(x => x.id === id);
    if (f) { (f as any)[key] = value; }
    this.dispatchEvent(new CustomEvent('ux:filter.action', { bubbles: true, composed: true, detail: { action: 'UPDATE', filters: [...this.filters] } }));
  }
