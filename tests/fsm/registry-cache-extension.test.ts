...
describe('FSMRegistry Cache Extension - Phase 1.2', () => {
  ...
  describe('getServiceCache()', () => {
    ...
    it('should throw on invalid service name', () => {
      ...
      expect(() => FSMRegistry.getServiceCache(null as any)).toThrow('[UX3] Service name must be a non-empty string');
      expect(() => FSMRegistry.getServiceCache(undefined as any)).toThrow('[UX3] Service name must be a non-empty string');
    });
  });
