...
describe('DevServer runtime asset injection', () => {
    it('appends runtime tags into head and scripts', async () => {
        ...
        await fs.writeFile(path.join(generatedDir, 'types.ts'), 'export type Routes = any; export type Services = any; export type I18n = any;');
        ...
        if (bundleRel) {
            ...
            for (const f of files)
                if (f.endsWith('.css'))
                    styles.push(path.join(path.relative(temp, outputDir), f).replace(/\\/g, '/'));
        }
...
//# sourceMappingURL=dev-server-runtime.test.js.map