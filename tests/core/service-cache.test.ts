...
describe('ServiceCache - Phase 1.1', () => {
  describe('Basic Operations', () => {
    ...
    it('should support complex types', () => {
      ...
      const userCache = new ServiceCache<User>(5000);
  ...
  describe('TTL and Expiration', () => {
    it('should expire values after TTL', async () => {
      const cache = new ServiceCache<string>(50); // 50ms TTL
      cache.set('value');
    ...
    it('should reset expiration when value is updated', async () => {
      const cache = new ServiceCache<string>(100);
      cache.set('value1');
    ...
    it('should report remaining TTL correctly', async () => {
      const cache = new ServiceCache<string>(1000);
      cache.set('value');
    ...
    it('should return 0 remaining TTL when stale', async () => {
      const cache = new ServiceCache<string>(10);
      cache.set('value');
  ...
  describe('Staleness Detection', () => {
    it('should detect when cache is stale', async () => {
      const cache = new ServiceCache<string>(50);
  ...
  describe('Invalidation', () => {
    ...
    it('should pass cache instance to observer', () => {
      cache.set('value');
      let receivedCache: ServiceCache<string> | null = null;
  ...
  describe('Clear Operation', () => {
    it('should clear cache like invalidate', () => {
      const cache = new ServiceCache<string>(5000);
      cache.set('value');
  ...
  describe('TTL Configuration', () => {
    ...
    it('should handle very large TTL values', () => {
      ...
      const cache = new ServiceCache<string>(YEAR_MS);
      cache.set('value');
  ...
  describe('Type Safety', () => {
    it('should maintain type safety with generics', () => {
      ...
      const productCache = new ServiceCache<Product>(5000);
    ...
    it('should work with array types', () => {
      const arrayCache = new ServiceCache<string[]>(5000);
      const items = ['a', 'b', 'c'];
    ...
    it('should work with union types', () => {
      ...
      const cache = new ServiceCache<CacheValue>(5000);
  ...
  describe('Edge Cases', () => {
    it('should handle null values properly (distinguish from no value)', () => {
      const cache = new ServiceCache<string | null>(5000);
    ...
    it('should handle rapid get/set operations', () => {
      const cache = new ServiceCache<number>(5000);
  ...
  describe('Real-world Usage Patterns', () => {
    it('should work as service response cache', async () => {
      ...
      const cache = new ServiceCache<UserResponse>(60000); // 60 second cache
    ...
    it('should support cache invalidation patterns', () => {
      const cache = new ServiceCache<string>(60000);
      cache.set('data v1');
    ...
    it('should support cache chain invalidation', () => {
      const userCache = new ServiceCache<{ id: number; name: string }>(60000);
      const profileCache = new ServiceCache<{ bio: string }>(60000);
