...
describe('navigateTo: window.history.pushState', () => {
    it('calls pushState with the correct path and view on navigation', () => {
        ...
        setupNavigation(ctx);
        navigateTo('/news', ctx);
        expect(pushState).toHaveBeenCalledWith(expect.objectContaining({ view: 'news', path: '/news' }), '', '/news');
...
describe('canNavigate guard', () => {
    ...
    it('can block specific views while allowing others', () => {
        const ctx = makeCtx();
        ctx.nav.canNavigate = (view) => view !== 'market';
...
//# sourceMappingURL=navigation-handler.test.js.map