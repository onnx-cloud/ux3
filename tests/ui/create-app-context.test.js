...
describe('createAppContext development logging', () => {
    ...
    it('should attach telemetry function when debug logging', async () => {
        const cfg = { ...baseConfig, development: { logging: 'debug' } };
        await createAppContext(cfg);
        expect(typeof window.__ux3Telemetry).toBe('function');
    });
    it('should expose inspector on window when enabled', async () => {
        ...
        expect(window.__ux3Inspector).toBe(ctx);
    });
    ...
    it('inspector widget should update when machine state changes', async () => {
        ...
        const ctx = await createAppContext(cfg);
        const widget = document.body.querySelector('ux3-inspector');
...
describe('AppContext helper methods', () => {
    ...
    it('registerView makes template available via context.template', async () => {
        const cfg = { ...baseConfig, templates: {} };
        const ctx = await createAppContext(cfg);
    ...
    it('registerMachine and registerRoute interact with router', async () => {
        ...
        expect(ctx.nav.routes.find((r) => r.path === '/foo')).toBeTruthy();
    });
    it('registerPlugin invokes install method on provided plugin', async () => {
        const cfg = { ...baseConfig };
        const ctx = await createAppContext(cfg);
        let called = false;
        const plg = { install(app) { called = !!app; } };
    ...
    it('loads and installs plugins declared in config.plugins', async () => {
        ...
        const cfg = { ...baseConfig, plugins: [chartsPath], site: {} };
        const ctx = await createAppContext(cfg);
        ...
        expect(ctx.nav.routes.find((r) => r.path === '/charts')).toBeTruthy();
    });
    it('auto-installs content plugin when config.content exists', async () => {
        ...
        const cfg = { ...baseConfig, content: { items: [simpleItem] } };
        const ctx = await createAppContext(cfg);
        // content plugin should register a route for /foo
        expect(ctx.nav.routes.some((r) => r.path === '/foo')).toBe(true);
...
//# sourceMappingURL=create-app-context.test.js.map