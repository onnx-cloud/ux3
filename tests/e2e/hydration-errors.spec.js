...
test.describe('Hydration timing and lifecycle', () => {
    test('hydration waits for DOMContentLoaded, not just page load', async ({ page }) => {
        ...
        await page.addInitScript(() => {
            window.domContentLoadedTime = 0;
            window.hydrationStartTime = 0;
        ...
        const timing = await page.evaluate(() => {
            return {
                dclTime: window.domContentLoadedTime,
    ...
    test('page does not load with stale __ux3App from previous navigation', async ({ page }) => {
        ...
        await page.waitForFunction(() => !!window.__ux3App, { timeout: 10000 });
        const firstAppId = await page.evaluate(() => window.__ux3App?.id || Math.random());
        ...
        const appValid = await page.evaluate(() => !!(window.__ux3App));
...
//# sourceMappingURL=hydration-errors.spec.js.map