 ...
 */
export async function waitForFSMState(page: Page, expectedState: string, timeout = 5000) {
  const startTime = Date.now();

  while (Date.now() - startTime < timeout) {
    const state = await page.evaluate(() => {
      return (window as any).__ux3App?.machines?.[Object.keys((window as any).__ux3App?.machines || {})[0]]?.getState?.();
    });
 ...
 */
export async function waitForI18n(page: Page, key: string, timeout = 5000) {
  const startTime = Date.now();

  while (Date.now() - startTime < timeout) {
    const text = await page.evaluate((k) => {
      return (window as any).__ux3App?.i18n?.(k);
    }, key);
 ...
 */
export async function getFSMContext(page: Page, fsmId: string) {
  return page.evaluate((id) => {
    return (window as any).__ux3App?.machines?.[id]?.getContext?.();
  }, fsmId);
}
 ...
 */
export async function sendFSMEvent(page: Page, fsmId: string, event: any) {
  return page.evaluate(
    ([id, evt]) => {
      (window as any).__ux3App?.machines?.[id]?.send?.(evt);
    },
    [fsmId, event]
  );
}
 ...
 */
export async function testErrorHandling(page: Page, triggerError: () => Promise<void>) {
  // Trigger error
  await triggerError();
