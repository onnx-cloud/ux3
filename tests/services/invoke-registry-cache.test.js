...
describe('InvokeRegistry - Caching (Phase 1.3.1)', () => {
    ...
    describe('Cache Configuration', () => {
        ...
        it('should respect custom cache TTL in options', async () => {
            ...
            const result1 = await registry.executeServiceInvoke({ service: 'api', method: 'getUser', input: { id: 1 } }, undefined, options);
            expect(result1.success).toBe(true);
            expect(callCount).toBe(1);
            // Call again immediately - should use cache
            const result2 = await registry.executeServiceInvoke({ service: 'api', method: 'getUser', input: { id: 1 } }, undefined, options);
    ...
    describe('Cache Hit/Miss', () => {
        ...
        it('should call service again when cache is disabled', async () => {
            ...
            // First call
            await registry.executeServiceInvoke({ service: 'api', method: 'getUser', input: { id: 1 } }, undefined, options);
            expect(callCount).toBe(1);
        ...
        it('should treat different service methods as separate cache entries', async () => {
            ...
            await registry.executeServiceInvoke({ service: 'api', method: 'getPost', input: { id: 1 } }, undefined, options);
        ...
        it('should treat different services as separate cache entries', async () => {
            ...
            await registry.executeServiceInvoke({ service: 'cache_service', method: 'slowFetch', input: { key: 'test' } }, undefined, options);
            expect(callCount).toBe(2); // New call needed
        });
    });
    ...
    describe('Custom Cache Keys', () => {
        it('should support custom cache keys', async () => {
            ...
            await registry.executeServiceInvoke({ service: 'api', method: 'getUser', input: { id: 1 } }, undefined, options1);
            await registry.executeServiceInvoke({ service: 'api', method: 'getUser', input: { id: 1 } }, undefined, options2);
    ...
    describe('Source Function Caching', () => {
        it('should cache source function results', async () => {
            ...
            // First call
            const result1 = await registry.executeInvoke({ src: myFunction }, undefined, options);
            // Second call
            const result2 = await registry.executeInvoke({ src: myFunction }, undefined, options);
    ...
    describe('Cache with Errors', () => {
        it('should not cache failed invocations', async () => {
            ...
            // First call - fails
            const result1 = await failRegistry.executeServiceInvoke({ service: 'error_service', method: 'throwError' }, undefined, options);
            expect(result1.success).toBe(false);
            expect(callCount).toBe(1);
            // Second call - should not be cached, new error
            const result2 = await failRegistry.executeServiceInvoke({ service: 'error_service', method: 'throwError' }, undefined, options);
    ...
    describe('Cache with Retry Logic', () => {
        it('should cache result after successful retry', async () => {
            ...
            await retryRegistry.executeServiceInvoke({ service: 'retry_service', method: 'mayFail' }, undefined, options);
...
//# sourceMappingURL=invoke-registry-cache.test.js.map