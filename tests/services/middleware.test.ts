...
describe('Service Middleware System', () => {
  ...
  describe('Logging Middleware', () => {
    it('should log successful HTTP requests', async () => {
      ...
      const mockNext = vi.fn(async (): Promise<ServiceResponse> => ({
        method: 'GET',
        status: 200,
        data: { id: 1 },
        ok: true,
    ...
    it('should log errors in requests', async () => {
      ...
      const request: RequestConfig = { method: 'POST', baseUrl: 'https://api.test.com' };
  ...
  describe('Request Mutation Middleware', () => {
    it('should detect when requests are mutated', async () => {
      ...
      const mockNext = vi.fn(async (req: RequestConfig) => {
        // Mutate the request
  ...
  describe('Header Augmentation Middleware', () => {
    it('should add custom headers to requests', async () => {
      ...
      const capturedRequest: RequestConfig = {};
      const mockNext = vi.fn(async (req: RequestConfig) => {
  ...
  describe('Middleware Integration Patterns', () => {
    it('should allow middleware to modify responses', async () => {
      const responseModifyingMiddleware: Middleware = async (req, next) => {
      ...
      expect((response.data as any).modified).toBe(true);
    });

    it('should allow early response returns (caching)', async () => {
      const cachingMiddleware: Middleware = async (req, next) => {
        if (req.method === 'GET' && (req as any).cachedResponse) {
          return (req as any).cachedResponse;
        }
        return next(req);
      };
      ...
      (request as any).cachedResponse = cachedResponse;
