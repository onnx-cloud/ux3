 ...
 */
export function createTestFSM(machineConfig) {
 ...
 */
export function waitForState(fsm, targetState, timeoutMs = 1000) {
 ...
 */
export function collectTransitions(fsm, duration = 100) {
    return new Promise((resolve) => {
        const states = [fsm.getState()];
        const unsubscribe = fsm.subscribe((state) => {
            states.push(state);
        });
        setTimeout(() => {
 ...
 */
export function assertTransition(fsm, fromState, event, expectedState) {
    if (fsm.getState() !== fromState) {
        throw new Error(`Expected FSM in state "${fromState}", got "${fsm.getState()}"`);
    }
    fsm.send(event);
    if (fsm.getState() !== expectedState) {
        throw new Error(`Expected transition from "${fromState}" via "${event}" to "${expectedState}", got "${fsm.getState()}"`);
    }
}
/**
 ...
 */
export function createTestForm(fields) {
...
export const testScenarios = {
     ...
     */
    failedLoginWithRetry: async (fsm) => {
        const states = [];
        fsm.subscribe((state) => states.push(state));
     ...
     */
    editAndSaveAccount: async (accountFSM, newProfile = testData.updatedProfile) => {
        const states = [];
     ...
     */
    sendChatMessage: async (chatFSM, message = testData.chatMessage) => {
        const states = [];
...
export const assertions = {
     ...
     */
    canSendEvent(fsm, event, config) {
        const currentState = fsm.getState();
        const stateConfig = config.states?.[currentState];
        if (!stateConfig)
            return false;
        if (typeof stateConfig === 'string')
            return false; // No transitions in short form
     ...
     */
    hasContextProps(fsm, expectedProps) {
     ...
     */
    contextHasValue(fsm, prop, value) {
        const context = fsm.getContext();
