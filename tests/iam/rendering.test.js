...
const mockAppContext = {
    machines: Object.fromEntries(Object.entries(config.machines).map(([key, machineConfig]) => [
        key.replace('FSM', '').toLowerCase(),
        new StateMachine(machineConfig)
    ])),
    services: {},
    styles: {
...
describe('IAM View Rendering', () => {
    beforeEach(() => {
        ...
        window.__ux3App = mockAppContext;
        // Reset FSMs
        Object.values(mockAppContext.machines).forEach((fsm) => {
    ...
    describe('Login View', () => {
        ...
        beforeEach(() => {
            ...
            element = document.createElement('ux-login');
    ...
    describe('Account View', () => {
        it('should render viewing state', (done) => {
            const fsm = mockAppContext.machines.account;
            let transitionCount = 0;
            fsm.subscribe((state) => {
                transitionCount++;
